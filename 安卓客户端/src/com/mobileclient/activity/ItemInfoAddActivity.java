...
public class ItemInfoAddActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.iteminfo_add); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("添加评价指标");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_itemTitle.getText().toString().equals("")) {
						Toast.makeText(ItemInfoAddActivity.this, "指标标题输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_itemDesc.getText().toString().equals("")) {
						Toast.makeText(ItemInfoAddActivity.this, "指标描述输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传评价指标信息*/
					ItemInfoAddActivity.this.setTitle("正在上传评价指标信息，稍等...");
					String result = itemInfoService.AddItemInfo(itemInfo);

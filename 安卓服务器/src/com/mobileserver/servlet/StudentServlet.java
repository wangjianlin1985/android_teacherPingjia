...
public class StudentServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (action.equals("query")) {
			...
			qq = qq == null ? "" : new String(request.getParameter(
					"qq").getBytes("iso-8859-1"), "UTF-8");

			/*调用业务逻辑层执行学生信息查询*/
			List<Student> studentList = studentDAO.QueryStudent(studentNumber,name,sex,telephone,qq);
			...
			for (int i = 0; i < studentList.size(); i++) {
				...
				.append("</sex>").append("\r\n")
				.append("		<age>")
				.append(studentList.get(i).getAge())
				.append("</age>").append("\r\n")
				.append("		<telephone>")
				.append(studentList.get(i).getTelephone())
				.append("</telephone>").append("\r\n")
				.append("		<qq>")
				.append(studentList.get(i).getQq())
				.append("</qq>").append("\r\n")
				.append("		<address>")
			...
			try {
			  ...
			  stringer.key("age").value(student.getAge());
			  stringer.key("telephone").value(student.getTelephone());
			  stringer.key("qq").value(student.getQq());
